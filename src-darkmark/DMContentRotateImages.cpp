...
//	std::time_t timestamp_of_last_scrollfield_rebuild = std::time(nullptr);
	...
	try
	{
		for (size_t idx = 0; idx < original_max_idx; idx ++)
		{
			...
			new_fn = original_file.getFileName();
			...
			for (const auto rotation : {cv::ROTATE_90_CLOCKWISE, cv::ROTATE_180, cv::ROTATE_90_COUNTERCLOCKWISE})
			{
				...
				new_fn = original_fn + "_r" + (rotation == cv::ROTATE_90_CLOCKWISE ? "090" : rotation == cv::ROTATE_180 ? "180" : "270");
				...
				cv::imwrite(new_full_fn, dst, {cv::IMWRITE_JPEG_QUALITY, 75});
				...
				if (original_marks.empty() == false or content.image_is_completely_empty)
				{
					...
					Log("creating rotated annotations for " + new_full_fn);
					...
					Log("reloading rotated image " + new_full_fn);
